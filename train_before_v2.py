...
from models.rnp_mamba import RNPMambaV1, RNPMambaV11, RNPMambaV12, RNPMambaV11Diag, RNPMambaV12Diag
...
def main():
    ...
    parser.add_argument('--model', type=str, default='stid_core', choices=['stid_core', 'rnp_mamba_v1', 'rnp_mamba_v11', 'rnp_mamba_v12', 'rnp_mamba_v11_diag', 'rnp_mamba_v12_diag'])
    ...
    parser.add_argument("--fusion_mode", type=str, default="gate", choices=["gate", "rec", "non", "avg"])
    ...
    elif args.model == "rnp_mamba_v11_diag":
        model = RNPMambaV11Diag(
            num_nodes=num_nodes,
            input_len=args.input_len,
            output_len=args.output_len,
            d_model=args.rnp_d_model,
    ...
    elif args.model == "rnp_mamba_v12_diag":
        model = RNPMambaV12Diag(
            num_nodes=num_nodes,
            input_len=args.input_len,
            output_len=args.output_len,
            d_model=args.rnp_d_model,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if hasattr(model, "get_gate_stats"):
            ...
            if gate_stats is not None:
                print(
                    "val/gate_stats: "
                    f"mode={gate_stats['fusion_mode']}, "
                    f"mean={gate_stats['gate_mean']:.4f}, "
                    f"std={gate_stats['gate_std']:.4f}, "
                    f"min={gate_stats['gate_min']:.4f}, "
                    f"max={gate_stats['gate_max']:.4f}"
                )
